...
public:
    ...
    void Config(uint8_t bw, uint8_t sf, uint8_t cr, uint32_t freq, uint8_t PreambleLength, bool InvertIQ, uint8_t PayloadLength, uint32_t interval);
...
private:
    ...
    SetDio3AsTcxoControl(uint8_t OutputVoltage, uint32_t delay_us);
